import logging
logging.basicConfig(filename='trello_search_template_checklist_new.log', level=logging.INFO)
...
class trello_search_template_checklist_new(NebriOS):
    ...
    def check(self):
        ...
        if type(self.trello_search_template_checklist_new) == str:
            if not self.trello_search_template_checklist_new.startswith('RAN'):
                self.drip = self.trello_search_template_checklist_new
                return True
        return False

    def action(self):
        self.trello_search_template_checklist_new = 'RAN: {}'.format(datetime.now())
